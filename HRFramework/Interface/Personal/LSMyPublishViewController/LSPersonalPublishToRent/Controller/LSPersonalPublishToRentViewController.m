...
@interface LSPersonalPublishToRentViewController ()<LSPersonalPublishToRentCellDElegate>
{
    BOOL isFirst;
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    LSGetMyWantHouseListModel *model = self.dataSourceArr[indexPath.row];
    
    LSNeedOfficeRentDetaiViewController *needOfficeRentDetaiVC = [[LSNeedOfficeRentDetaiViewController alloc] initWithNibName:@"LSNeedOfficeRentDetaiViewController" bundle:nil];
    
    needOfficeRentDetaiVC.Id = model.wantId;
    needOfficeRentDetaiVC.itemTypeId = @"4";
    needOfficeRentDetaiVC.hiddenLowView = 45465;
    
    [self.navigationController pushViewController:needOfficeRentDetaiVC animated:YES];
...
-(void)requestGetMyWantHouse:(BOOL)isMore{
    ...
    [[HRRequestManager manager] GET_PATH:PATH_GetMyWantHouse para:para success:^(id responseObject) {
        ...
        if ([responseObject[@"success"]boolValue]== 1) {
            ...
            [self requestWithFindMyWantHouseListModelProject:[responseObject objectForKey:@"ds"] isMore:isMore];
...
- (void)requestWithFindMyWantHouseListModelProject:(NSArray *)Arr isMore:(BOOL)isMore
{
    if (!isMore) {
    ...
    for (NSDictionary *dic in Arr) {
        ...
        LSGetMyWantHouseListModel *model = [[LSGetMyWantHouseListModel alloc] initWithDictionary:dic];
...
-(void)alertActionMethod:(NSInteger)indexPath andIsSuccess:(NSInteger)issuccess{
    ...
    LSGetMyWantHouseListModel *model = self.dataSourceArr[indexPath];
    NSMutableDictionary *para = [NSMutableDictionary dictionary];
    [para setObject:model.wantId forKey:@"itemId"];
    [para setObject:@"4" forKey:@"itemTypeId"];
